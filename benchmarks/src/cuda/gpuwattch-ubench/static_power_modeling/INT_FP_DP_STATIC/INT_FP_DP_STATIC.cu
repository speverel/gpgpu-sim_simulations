...
__global__ void power_microbench(float *data1, double *data2, uint32_t *data3, uint32_t *data4, float *res, int div, uint64_t iterations) {

  int gid = blockIdx.x*blockDim.x + threadIdx.x;
  register float s1 = data1[gid];
  register double s2 = data2[gid];
  register uint32_t s3 = data3[gid];
  register uint32_t s4 = data4[gid];

  register float Value1=0;
  register uint32_t Value2=0;
  register double Value3=0;
  ...
  //ROI
    ...
    for (uint64_t j=0 ; j<iterations ; ++j) {
      asm volatile ("{\t\n"
          ...
          "fma.rn.f64 %5, %5, %5 , %4;\n\t"
          "mad.lo.u32 %3, %3, %3 , %2;\n\t"
          "}" : "+f"(Value1),"+f"(s1),"+r"(Value2),"+r"(s3),"+d"(Value3),"+d"(s2)
      );
      // result=s1+s2;
      // Value2=s1-s2;
      // result+=Value1;
  ...
  res[gid] = (float)Value1 + (float)Value2 + (float)Value3;
}
