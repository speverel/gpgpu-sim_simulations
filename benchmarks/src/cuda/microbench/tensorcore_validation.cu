...
int main(int argc, char **argv)
{
    ...
    checkCudaErrors(cudaMalloc((void**)&A_h, sizeof(float) * M_GLOBAL * K_GLOBAL));
    checkCudaErrors(cudaMalloc((void**)&B_h, sizeof(float) * K_GLOBAL * N_GLOBAL));
    checkCudaErrors(cudaMalloc((void**)&C_h, sizeof(float) * M_GLOBAL * N_GLOBAL));
    ...
    A_h      = (float*)malloc(M_GLOBAL * K_GLOBAL * sizeof(float));
    B_h      = (float*)malloc(K_GLOBAL * N_GLOBAL * sizeof(float));
    ...
    checkKernelErrors((init_device_matrices<<<80, THREADS_PER_BLOCK>>>(A_h, B_h, C_h, A, B, C, D)));
    ...
    checkKernelErrors((compute_gemm<<<80, THREADS_PER_BLOCK>>>(A, B, C, D, alpha, beta)));
