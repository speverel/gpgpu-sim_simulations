...
# Libs
LIB       := -L$(CUDA_INSTALL_PATH)/lib -L$(LIBDIR) -L$(COMMONDIR)/lib/$(OSLOWER) -L/home/vkz4947/cuda-9.1/lib64 -L/usr/lib64 -lstdc++
ifeq ($(USEDRVAPI),1)
...
$(OBJDIR)/%.cu_o : $(SRCDIR)%.cu $(CU_DEPS)
	$(VERBOSE)$(NVCC) $(NVCCFLAGS) -gencode=arch=compute_70,code=\"sm_70,compute_70\"  $(SMVERSIONFLAGS) -o $@ -c $<

$(CUBINDIR)/%.cubin : $(SRCDIR)%.cu cubindirectory
	$(VERBOSE)$(NVCC) $(CUBIN_ARCH_FLAG) -gencode=arch=compute_70,code=\"sm_70,compute_70\"  $(NVCCFLAGS) $(SMVERSIONFLAGS) -o $@ -cubin $<
